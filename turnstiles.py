...
class TurnStiles:
    ...
    def week_ridership(self, data):
        '''find the total ridership each week  for each station'''
        stations = {}
        for key in data:
            total = 0
            for day in data[key]:
    ...
    ########################################################################### 
    ## Everything below this line is just here to make the above bits of     ##
    ## code run smoothly, one after another                                  ##
    ###########################################################################
    ...
    def run_all(self, multiple=False):
        #self.download_data("turnstile_1506", 6, 30)
